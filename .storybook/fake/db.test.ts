...
describe("the busy fault", () => {
  ...
  it("refuses every write there is, and no read", () => {
    ...
    //
    // The quick add then added **one**, 89 → 90: `deck_quick_add_to_collection` is the fourth
    // write to reach across the deck boundary and the first that *creates* a `collection_entries`
    // row rather than moving one, so it takes `collection_source::with_write_owned` for the
    // strongest version of its three neighbours' reason — the facet index's `owned` dimension is
    // built by counting rows and this write makes one. Its read half
    // (`deck_quick_add_wishes`) goes through `lock_db_read` and is not in this table at all,
    // exactly as `deck_pull_plan` is not. Re-counted by running the sweep on this branch; whoever
    // merges it next should run it again rather than trusting this line.
