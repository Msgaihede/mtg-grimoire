...
export interface FakeDeck {
  ...
  /**
   * `decks.theory_mark_exact` and `decks.theory_mark_name` (schema v36): which of the theory
...
function toDeckRow(db: FakeDb, d: FakeDeck): DeckRow {
  return {
    ...
    // v36's pair, and the **fifth and sixth** columns on the `?? default` footing — but the
