...
**Three of the exclusions are permanent** (spec §6.3): the plain-text `mirror`, the Rust
`transfer` writer that exists only for it, and `window`'s Win32 snap layouts. The rest are
"not yet" — they arrive with the commands that need them. `images` is neither: on web the
image cache is Cache Storage rather than a filesystem, so it is a rewrite and not a port.

**`update` used to be the fourth permanent one and only half of it ever was.** §6.3 named
"the portable `update` swap", and the swap is exactly what stayed: the `.exe` replacement,
the staging, the digest check and the relaunch are all gated item by item, along with
`Updater` itself — its `reqwest` client sets `user_agent`, `connect_timeout` and
`read_timeout`, none of which the wasm backend has. What crossed is the half that *reports*:
`UpdateStatus`, `update::history` and the version comparison under them are a `serde` struct
and two `app_meta` reads. That is what lets a browser answer `update_status` and
`update_history`, which is what makes the Updates panel decidable there — see
"2026-08-31: the Updates panel comes back" below.
...
| Routed | **114** *(**119** after 2026-08-31 — see below)* |
| Not routed | **40** *(**35**)* |
...
| **7** | `desktop.rs` | The five §6.3 updater commands, `update_status`, and `sync_run` — the web target runs its own ingest through `glue.rs`. **Two of the seven were routed on 2026-08-31**: `update_status` and `update_history` report rather than replace |
| **5** | `reset.rs` (4) and `deck_set_cover_image` | OPFS deletion, and a covers directory a browser has none of. **Three of the four clears were routed on 2026-08-31** — they are pure SQLite and always were; only `cache_clear` sweeps files |
