...
`deck_tokens.rs` and user schema **v35**,
...
`deck_tokens` is the **thirteenth** synced table (`schema::SYNCED_TABLES`, user schema v35), and
...
v35's second half is `decks.tokens_open INTEGER NOT NULL DEFAULT 0` — whether the editor's
...
`IMAGE_COL` is where they start — it read **21** until v35 and reads **22** now. Forgetting to move
