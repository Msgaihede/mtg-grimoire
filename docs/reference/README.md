...
| [search-syntax.md](search-syntax.md) | Scryfall query syntax in the card search boxes — the fourteen keywords and their per-keyword default operator, why `t:`/`o:` ride FTS (82× and 277×), the one function that reaches all three searches, corpus schema 5 and `kw:`, and the one arm that fails closed | [`src/CLAUDE.md`](../../src/CLAUDE.md) |
