...
mod tests {
    ...
    fn every_advertised_command_is_actually_routed() {
        ...
        //
        // **157 since the deck's notebook** — one read, one card-side read and six writes, the
        // whole of `deck_notes`. Written from 149 plus eight *and* counted off the array, which
        // is the discipline the paragraphs above ask for; if a merge turns this red, take the
        // number from `left`.
        assert_eq!(
            COMMANDS.len(),
            157,
