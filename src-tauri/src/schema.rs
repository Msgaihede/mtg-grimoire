...
/// and which the app chose for them. 36 is the second rung here that writes no shape: it sweeps
/// every `kind = 'deck'` folder and files whatever no **live** `deck_cards` row claims at
/// `(card_id, finish)` into `Recently removed`, which is the one-time pass that brings a file
/// the v25 conversion filed by oracle id under the grain `deck::owned_by_printing` now counts
/// at.
/// 37 is the thirteenth synced table, `deck_tokens`, and one more column on `decks`: which
/// tokens and emblems a deck brings is *derived* from its cards' `all_parts` on every open,
/// so the table holds only the reader's deviations from that — a picked printing, a stepped
/// quantity, a dismissal — and `tokens_open` is whether the area under the deck is expanded,
/// which joins `last_variant` and its three neighbours as per-deck view state. 38 is two
...
/// and the state a reader wants here is both marks on. 39 is one more column beside those two,
/// `theory_mark_unplanned` — the third tier, and the one worn by a live row the plan does *not*
/// ask for at all: a stand-in, a spare or an experiment, marked in red and carrying no number
/// because there is no planned count to compare it against. **`NOT NULL DEFAULT 1` for 38's own
/// reason**, which one more rung has not changed: the default is what the reader gets, and the
/// state a reader wants here is the mark on. One column and not a widening of either of 38's,
/// for that rung's own "two rather than one three-valued" argument read a tier further — the
/// three switches are independent, and a reader who wants only this one is asking a coherent
/// question. 40 is one column on `decks`, `virtual_only` — the third deck kind, for a deck the
/// reader tracks without owning cardboard for it (MTGO, Arena, proxies). **`NOT NULL DEFAULT
/// 0`**, which is 34's choice rather than 38's and made for that rung's reason: every deck that
/// already exists is a deck about cardboard, so the upgrade is invisible and no reader's deck
/// changes kind under them. **A second boolean rather than a `kind` column**, which is 38's
/// "two rather than one three-valued" argument read from the other end: the three kinds are
/// derived from `theory_enabled` and this one — `0/0` regular, `1/0` theory-and-actual, `0/1`
/// virtual — so a rung that stored the enum would have had to rewrite `theory_enabled` on every
/// row and leave two columns able to disagree. `1/1` is the one pair with no meaning, and
/// `deck::update_deck` is where it is made unreachable rather than a `CHECK`, because
/// `ALTER TABLE … ADD COLUMN` cannot carry a table-level one and a refusal in Rust can say why.
///
/// **Both of the last two rungs were written as 35 and renumbered on the way in — the token
/// rung twice, the theory rung three times.** The sixth grade took 35, the deck-group sweep
/// took 36 and the token rung took 37, all while more than one branch was open, and the number
/// belongs to whoever lands first — exactly as v34 was renumbered off v33 and as v12/v13/v14
/// collided three ways in one day. **The sharpest thing either branch learned is that this
/// constant is the one line git cannot help with**: each time, both sides read the *same*
/// number here, so there was no conflict on it at all and only the rungs underneath collided.
/// Three renumbers on one branch is the record, and it is the clearest argument the ladder has
...
///
/// **43 (2026-09-10, [#447](https://github.com/Msgaihede/mtg-grimoire/issues/447)) is the
/// fourteenth and fifteenth synced tables, `deck_notes` and `deck_note_cards`, and it is the
/// first rung on this ladder that takes a column *away*.** A deck had one `notes` paragraph
/// since v8, reachable only through a textarea in a dialog; it now has as many notes as the
/// reader writes, each able to name any number of cards. The card reference is a pointer the
/// note holds rather than a place the note lives — so the notes list is complete by
/// construction and a note cannot become invisible by acquiring a card.
///
/// **`deck_note_cards` attaches by `oracle_id` and not `card_id`**, [`DECK_TOKEN_GRAIN`]'s
/// argument verbatim: a printing id means nothing on the far device's shelf, an oracle id is
/// Scryfall's and is the same everywhere, so a note survives the reader swapping printings and
...
/// `deck_audit`; `deck_note_cards` gets one for the opposite reason. Two devices each typing a
/// note about the mana base must stay two notes, and there is no column pair that could tell an
/// accidental duplicate from a deliberate one — while two devices attaching Lightning Bolt to
/// one note describe a single fact, which is what `idx_deck_note_cards_grain` folds.
///
/// ⚠️ **`notes_open` is `DEFAULT 0`, which is v37's answer rather than v42's.** The two rungs
/// asked one question and answered it opposite ways, and which one applies turns on whether the
/// upgrade changes what is on screen. The stats band was already there for every deck on every
/// disk, so v42 wrote `1`; the Notes band is *new* and no deck has ever shown one, so a
/// collapsed default takes nothing from anybody and `decks.tokens_open` is the precedent
/// character for character.
///
/// **The paragraph already in `decks.notes` is discarded rather than migrated**, decided the
/// day the rung was written. A copy would put prose nobody asked to keep into a list whose whole
/// point is that its rows are deliberate. What that costs is said here rather than hidden: a
...
pub const TABLES: &[(&str, Side)] = &[
    ...
    // `SYNCED_TABLES` answers that one, and this table is not on it.
    ...
    // Which cards a note names (user schema v43). By `oracle_id`, so the row means the same
    // thing on the far device's shelf — `deck_tokens`' argument one table down. Nothing
    // rebuilds it: a feed knows every printing of a card and none of them knows the reader
...
pub const SYNCED_TABLES: [&str; 15] = [
    ...
    "deck_folders",
    "deck_labels",
    // The fifteenth (user schema v43), and the one on this list with a *grain* rather than
    // only a uid: two devices attaching one card to one note describe a single fact, so the
    // applier resolves on `(note_uid, oracle_id)` and the two converge with no alias table.
    // `deck_notes` beside it deliberately has none — see [`USER_SCHEMA_VERSION`]'s v43
    // paragraph, which is the same question answered the other way.
    "deck_note_cards",
    // The fourteenth (user schema v43). The reader's prose about a deck, which is as
...
///
/// **`note_id` and not `deck_id`**, which is the whole model in one line: a card reference is a
/// pointer the note holds, never a place the note lives, so the row hangs off the note and two
/// notes on one deck may name the same card without colliding.
///
/// **`oracle_id` and not `card_id`**, [`DECK_TOKEN_GRAIN`]'s argument verbatim: the row survives
/// the reader swapping to a different printing, a note written against the Theory list shows on
/// the Live one because both hold the same oracle id, and a note naming Lightning Bolt names it
/// once however many copies or finishes the deck holds.
///
/// **The grain is what makes the far device's insert idempotent.** Two devices attaching one
/// card to one note describe a single fact; without `idx_deck_note_cards_grain` the incoming op
/// is an insert that hits nothing, both rows land, and the card modal reads "2 notes" for one
/// note. With it the applier resolves on `(note_uid, oracle_id)`, takes `min(uid)`, and the two
/// converge with no alias table — the rule `docs/reference/sync.md` states and
/// `idx_deck_tokens_grain` already relies on. **`deck_notes` beside it deliberately carries no
/// grain at all**, and that is not an omission: two devices each typing a note about the mana
/// base must stay two notes.
///
/// Held to its index by
/// `every_plain_grain_constant_names_the_index_the_head_schema_carries`, which can read this one
/// through `PRAGMA index_info` because it carries no `coalesce`. It is **not** an `ON CONFLICT`
/// target — [`crate::deck_notes`] attaches with `INSERT OR IGNORE`, so the second fence
/// [`DECK_TOKEN_GRAIN`] has is not available here and this test is the whole of the fence.
pub const DECK_NOTE_CARD_GRAIN: &str = "note_id, oracle_id";
...
/// The twenty-seven user tables and their forty-four indexes, at [`USER_SCHEMA_VERSION`]'s
...
CREATE TABLE {schema}.deck_notes (
                 id INTEGER PRIMARY KEY,
                 deck_id INTEGER NOT NULL REFERENCES decks(id) ON DELETE CASCADE,
...
CREATE TABLE {schema}.deck_note_cards (
                 id INTEGER PRIMARY KEY,
...
CREATE UNIQUE INDEX {schema}.idx_deck_note_cards_grain ON deck_note_cards (note_id, oracle_id);

CREATE UNIQUE INDEX {schema}.idx_deck_notes_uid ON deck_notes (sync_uid);

CREATE UNIQUE INDEX {schema}.idx_deck_note_cards_uid ON deck_note_cards (sync_uid);
"#;

/// Create the twenty-seven user tables and their indexes in `schema`, at
...
fn migrate_user(conn: &Connection) -> rusqlite::Result<()> {
    ...
    // v36: a deck's group holds only what its live list claims at `(card_id, finish)`.
    //
    ...
    // would silently convert a v35 file differently next year. v25's rung inlines the same
    // arithmetic for the same reason.
    //
    // **It writes no shape at all, so it owes no `USER_SCHEMA_SQL` line and no `UNDO_V37`** —
    ...
    //
    // **It was written as 35 and renumbered to 36 on the way in**, v34's own note one rung down
    // and this ladder's rule rather than an accident worth hiding: the sixth grade took 35 while
    // this branch was open, and the number belongs to whoever lands first. **Here the order is
    // more than the rule, though** — v35 rebuilds `collection_entries`, and every statement below
    // reads and writes that table, so this rung has to sit after it and not merely below it.
    if v < 36 {
        let tx = conn.unchecked_transaction()?;
        ...
        // *makes* a database version 36. `USER_SCHEMA_VERSION` would commit "fully migrated"
        // before any step added after it had run.
        tx.execute_batch("PRAGMA main.user_version = 36;")?;
        tx.commit()?;
    }

    // v37: the tokens and emblems a deck brings, and whether the reader has that area open.
    //
    ...
    if v < 37 {
        ...
        // *makes* a database version 37. `USER_SCHEMA_VERSION` would commit "fully migrated"
        // before any step added after it had run.
        tx.execute_batch("PRAGMA main.user_version = 37;")?;
        tx.commit()?;
    }

    // v38: the theory mark grew a second tier, and a deck can now be told which of the two it
    ...
    //
    // **Both `NOT NULL DEFAULT 1`, and the default is the whole of the upgrade** — v34's line
    // one rung down, with the answer the other way round for the same argument. Every deck that
    // already exists draws both marks from the first launch on the new build, and there is no
    // backfill because there is nothing for one to do. The alternative — defaulting the new
    // tier off so nothing changes — leaves the headline half of the feature invisible until
    // somebody finds a switch they have no reason to look for, and splits the reader's decks
    // into two groups that behave differently with nothing on screen saying why.
    //
    // **Two columns rather than one three-valued one.** `none | exact | both` would have been
    // one `ALTER` and would have made the two tiers a ladder: it can spell "blue only" as
    // nothing at all, and blue without green is a real answer — a reader who cares that a card
    // is *present* and not which printing it is. Two booleans are the four states the switches
    // draw, one to one.
    //
    // **Per deck rather than per user**, `theory_enabled`'s own argument one column along, and
    // `separate_x_group`'s and `bracket`'s after it: it is an answer about how this list is
    // read, so two decks may disagree.
    //
    // **Appended at the end, which is what keeps `deck.rs`'s positional reads honest.** Every
    // `r.get(n)` over `DECK_SELECT` is an index into this column order, and both new columns
    ...
    if v < 38 {
        ...
        // Literal `38`, for the reason every step before it writes its own: this step is what
        // *makes* a database version 38. **After both `ALTER`s and inside the same
        // transaction** — a stamp between them would leave a launch that died in the middle at
        // version 38 with one column, and the next launch would run no rung to add the other.
        tx.execute_batch("PRAGMA main.user_version = 38;")?;
        tx.commit()?;
    }

    // v39: the theory mark grew a **third** tier, and it is the one about absence
    // (2026-09-08). The rung above draws green where a live row is the printing the plan named
    // and blue where it is the same card in one it did not; both are marks on a row the plan
    // *asks for*. A live row the plan does not mention at all — a stand-in, a spare, an
    // experiment — had no mark and no way to get one, and that is the state a reader building
    // against a plan most wants to see. It wears a red X and no number, because "how many does
    // the plan want" has no answer here, and the deck says whether it is drawn at all.
    //
    // **`NOT NULL DEFAULT 1`, for v38's reason unchanged one rung later**: every deck that
    // already exists draws the third mark from the first launch on the new build, there is no
    // backfill because there is nothing for one to do, and defaulting it off would hide the
    // whole of this rung behind a switch nobody has a reason to look for.
    //
    // **A third column rather than a widening of either of v38's**, which is that rung's "two
    // booleans and not one three-valued column" argument read a tier further: the three
    // switches are independent, the eight states they spell are all reachable, and a reader who
    // wants only the red X — show me what is *not* in the plan — is asking a coherent question
    // that no ladder-shaped encoding can answer.
    //
    // **Appended at the end**, which is what keeps `deck.rs`'s positional reads honest: every
    ...
    // this table spells its field list out literally and there is no fence in the other
    // direction, so a column added here and nowhere else is captured by nothing and goes red
    // nowhere. `DEFAULT 1` is what makes the old-peer direction safe — `apply::updates` walks
    // the local spec and `continue`s past a field an incoming op lacks, `apply::creations`
    // omits it from the INSERT and this DDL default fills it in, so a deck built from an op a
    // device on v38 sent arrives with the third mark on.
    if v < 39 {
        let tx = conn.unchecked_transaction()?;
        tx.execute_batch(
            "ALTER TABLE decks ADD COLUMN theory_mark_unplanned INTEGER NOT NULL DEFAULT 1;",
        )?;
        // Literal `39`, for the reason every step before it writes its own: this step is what
        // *makes* a database version 39. **After the `ALTER` and inside the same transaction** —
        // the stamp and the column commit together or neither does, which is v38's rule with one
        // statement instead of two rather than a rule it no longer needs.
        tx.execute_batch("PRAGMA main.user_version = 39;")?;
        tx.commit()?;
    }

    // v40: the third deck kind (2026-09-08). A **Virtual** deck is one the reader tracks
    // without owning cardboard for it — an MTGO or Arena list, a proxy pile, a deck they are
    // reading about. It keeps one `live` list and has no integration with the collection or the
    // wishlist at all: nothing it names is owned, missing, pullable or shoppable, because there
    // is no cardboard for any of those words to be about.
    //
    // **`NOT NULL DEFAULT 0`, which is v34's choice rather than v38's**, and made for v34's
    // reason read here: every deck already on a reader's disk is a deck about cardboard, so the
    // default is the state that leaves every existing deck exactly as it was and makes the
    // upgrade invisible. v38's argument — the default is what the reader gets, so default the
    // headline on — is the *opposite* answer to the same question and is right there because
    // the mark is a way of *reading* a deck; a kind is what a deck **is**, and no rung may
    // change that under a reader who did not ask.
    //
    // **A second boolean rather than a `kind` column, and the three kinds are derived from the
    // pair.** `theory_enabled` / `virtual_only` reads `0/0` regular, `1/0` theory-and-actual and
    // `0/1` virtual; `1/1` is the one pair with no meaning. Storing the enum instead would have
    // meant a rung that rewrites `theory_enabled` on every row — a column three commands, a
    // sync spec, an undo journal and a history payload already name — to gain nothing but a
    // second spelling of a fact those readers would then have to be taught. It is v38's "two
    // booleans, not one three-valued column" argument arriving from the other end, and the price
    // is the same one: two columns that *can* disagree.
    //
    // **The pair is fenced in Rust and not by a `CHECK`, and this rung could not have carried
    // one anyway.** SQLite's documented `ADD COLUMN` restrictions rule out a table-level CHECK,
    // and the column-level one that is legal cannot see `theory_enabled` — so the fence would
    // have had to be v35's kind of table rebuild for a rule one command can hold. `deck::
    // update_deck` is where it lives: a patch that sets either flag clears the other in the
    // *values it binds*, which is `valid_game`'s and `valid_bracket`'s standing — a refusal in
    // Rust can say why, and a command parameter reaches this column.
    //
    ...
    // still hold a `0` or a `1`.
    //
    // **`decks` is in [`SYNCED_TABLES`] and this column travels — but only because it was put on
    // the capture spec by hand**, v38's and v39's note verbatim: `sync_engine::capture`'s `Spec`
    // for this table spells its field list out literally and there is **no fence in the other
    // direction**, so a column added here and nowhere else is captured by nothing and goes red
    // nowhere. What a deck *is* is the strongest case on that list — a deck that is virtual on
    // the machine it was made on and a cardboard deck everywhere else would have its collection
    // integration back on every other device, offering to buy cards for a list that is not about
    // cards. `DEFAULT 0` is what makes the old-peer direction safe: `apply::updates` walks the
    // local spec and `continue`s past a field an incoming op lacks, `apply::creations` omits it
    // from the INSERT and this DDL default fills it in, so a deck built from an op a device on
    // v39 sent arrives as an ordinary deck — which is what a device that has never heard of the
    // kind can only have meant.
    if v < 40 {
        let tx = conn.unchecked_transaction()?;
        tx.execute_batch("ALTER TABLE decks ADD COLUMN virtual_only INTEGER NOT NULL DEFAULT 0;")?;
        // Literal `40`, for the reason every step before it writes its own: this step is what
        // *makes* a database version 40. **After the `ALTER` and inside the same transaction** —
        // the stamp and the column commit together or neither does, which is v39's rule
        // unchanged one rung later.
        tx.execute_batch("PRAGMA main.user_version = 40;")?;
        tx.commit()?;
    }
    ...
    // [`SYNCED_TABLES`] does not name it. What the table buys is a "shared" badge on a
    ...
    // v43: a deck gets as many notes as the reader writes, each able to name any number of
    // cards, and the single `decks.notes` paragraph it had since v8 goes
    // (2026-09-10, [#447](https://github.com/Msgaihede/mtg-grimoire/issues/447)).
    //
    // **`deck_notes` is v37's `deck_tokens` shape, and `deck_note_cards` hangs off *it* rather
    // than off `decks`.** That is the model rather than a storage detail: a card reference is a
    // pointer the note holds, never a place the note lives, so the notes list is complete by
    // construction and a note cannot become invisible by acquiring a card. `sync_uid` is written
    // on its own line before the close for v37's reason — both tables are born above the v29
    // rung, so the column is in the `CREATE` rather than in an `ALTER`, and the line break is
    // what keeps this and [`USER_SCHEMA_SQL`] byte-identical for
    // `the_user_schema_is_byte_identical_to_what_the_ladder_builds`.
    //
    // **One grain and not two.** `idx_deck_note_cards_grain` is owed because two devices
    // attaching Lightning Bolt to one note describe a single fact — without it the far op is an
    // insert that hits nothing, both rows land, and the card modal reads "2 notes" for one note.
    // `deck_notes` deliberately gets none: two devices each typing a note about the mana base
    // must stay two notes, and there is no column pair that could tell an accidental duplicate
    // from a deliberate one. See [`DECK_NOTE_CARD_GRAIN`], which is that argument in full.
    //
    ...
    // every disk and a `0` would have closed a band the reader had been reading for months. The
    // Notes band is *new* — no deck has ever shown one — so a collapsed default takes nothing
    // from anybody, and `decks.tokens_open` is the precedent character for character. The two
    // rungs are the same question answered opposite ways, and which one applies turns on
    // whether the upgrade changes what is on screen.
    //
    // **The old paragraph is discarded rather than migrated**, and nothing here copies
    // `decks.notes` anywhere. A copy would put prose nobody asked to keep into a list whose
    // whole point is that its rows are deliberate, and the column has never been shown anywhere
    // but one textarea in a dialog most readers never open. What that costs is stated rather
    // than hidden: a reader who used the old field loses it, with no undo, at the upgrade.
    //
    // **This is the first rung on the user ladder that takes a column away**, so it is also the
    // first whose rewind has to *put one back* — [`tests::UNDO_V43`] re-adds `notes TEXT`, and a
    // chain without that line lands near v42 rather than on it.
    if v < 43 {
        let tx = conn.unchecked_transaction()?;
        tx.execute_batch(
...
pub(crate) mod tests {
    ...
    fn a_full_sync_leaves_every_table_on_its_own_side() {
        ...
        assert_eq!(
            stray, 27,
            "the user file holds the twenty-seven and nothing else"
        );
    }
    ...
    #[test]
    fn the_user_side_is_the_twenty_seven_tables_no_feed_can_rebuild() {
    ...
    /// v43's two note tables and the column it took away — the newest rewind on the user
    /// ladder, and the first one on it that has to **add** something back.
    ///
    ...
    ///
    /// **It runs first, before [`UNDO_V42`]**, for that constant's stated reason: a rewind walks
    /// the ladder backwards and this is now the top of it. The doc directly below said the same
    /// of itself and told whoever wrote rung 43 to expect to collect the line; this is that
    /// line, and the sentence goes on being a prediction rather than history for exactly one
    /// rung at a time.
    ///
    ...
    /// [`UNDO_V38`], then [`UNDO_V35`], then [`UNDO_V34`] — and it read "one rewind, because
    ...
    /// to name a grade, and the day before a deck knew which tokens it brings. It is the
    /// population **both** the v35 and the v36 rungs are *for*.
    ///
    /// Rewound from head rather than written out a second time, [`user_file_at_33`]'s device
    /// one rung up. **Three rewinds do it, and it was written with one**: v35 was the only rung
    /// above 34 on the branch that added this, and v36, v37 and v38 have landed above it since.
    /// [`UNDO_V38`] first, then [`UNDO_V37`], then [`UNDO_V35`] — newest rung, first undone —
    /// with no `UNDO_V36` between the last two because v36 writes no shape.
    ///
    /// **It takes no `foreign_keys` parameter**, [`user_file_at_33`]'s reason, and the two
    /// rungs reach it by different routes. v35 rebuilds `collection_entries`, which nothing in
    /// the schema references, so the implicit `DELETE FROM` its `DROP TABLE` runs under
    /// `foreign_keys=ON` has no child row anywhere to reach. v36 creates a table nothing
    /// references and adds a column to one. Neither can behave two ways under that pragma. The
    /// CASCADE `deck_tokens` *declares* is a different question, and
    /// `v37_overrides_leave_with_the_deck_that_holds_them` turns the pragma on to ask it.
    ...
    /// A user file at 37 — the shape every machine carries the day before the theory mark grew
    /// a second tier, and the only population the v38 rung is *for*.
    ///
    /// Rewound from head rather than written out a second time, [`user_file_at_34`]'s device
    /// one rung up. It read "one rewind does it, because v38 is the only rung above 37", then
    /// **collected the line it was told to expect the moment v39 landed**, and has now
    /// collected a third the moment v40 did — [`UNDO_V40`] first, then [`UNDO_V39`], then
    /// [`UNDO_V38`], newest first, which is what every chain on this ladder does. Read the
    /// count as owing a line to whoever writes rung 41 rather than as a fact; twice in two days
    /// is not a coincidence.
    ///
    /// **It takes no `foreign_keys` parameter**, for the same reason the three below it do not:
    /// v38, v39 and v40 are `ADD COLUMN`s against `decks`, and no setting of that pragma can
    /// make an `ADD COLUMN` behave two ways.
    fn user_file_at_37() -> Connection {
        let conn = Connection::open_in_memory().unwrap();
        create_user_schema(&conn, "main").unwrap();
        conn.execute_batch(&format!(
    ...
    /// A user file at 38 — the shape every machine carries the day before the theory mark grew
    /// a *third* tier, and the only population the v39 rung is *for*.
    ///
    /// [`user_file_at_37`]'s construction one rung up. It read "one rewind does it because v39
    /// is the only rung above 38", with the sentence directly above recording what that claim
    /// was worth — and it was worth one day: [`UNDO_V40`] joined it when the deck kind landed,
    /// newest first. Read *this* count as owing a line to rung 41 too.
    ///
    /// **It takes no `foreign_keys` parameter**, for [`user_file_at_37`]'s reason: v39 and v40
    /// are single `ADD COLUMN`s against `decks`, and no setting of that pragma can make one
    /// behave two ways.
    fn user_file_at_38() -> Connection {
        let conn = Connection::open_in_memory().unwrap();
        create_user_schema(&conn, "main").unwrap();
        conn.execute_batch(&format!(
    ...
    /// A user file at 39 — the shape every machine carries the day before a deck could be a
    /// *kind*, and the only population the v40 rung is *for*.
    ///
    /// [`user_file_at_38`]'s construction one rung up, and one rewind does it because v40 is
    /// the only rung above 39. The two docs above record exactly what that claim has been worth
    /// on this ladder — one rung each, twice running — so read it as owing a line to rung 41
    /// rather than as a fact about the file.
    ///
    /// **It takes no `foreign_keys` parameter**, for [`user_file_at_37`]'s reason: v40 is a
    ...
    ///
    /// [`user_file_at_39`]'s construction two rungs up, and **it collected its second rewind
    /// when rung 43 landed** — exactly the line the paragraph here predicted it would owe, which
    /// makes four rungs running that the prediction has been right about.
    ///
    ...
    /// A user file at 42 — the shape every machine carries the day before a deck could hold more
    /// than one note, and the only population the v43 rung is *for*.
    ///
    /// [`user_file_at_41`]'s construction one rung up, and one rewind does it because v43 is the
    /// only rung above 42. The four docs above record exactly what that claim has been worth on
    /// this ladder — one rung each, four times running — so read it as owing a line to rung 44
    /// rather than as a fact about the file.
    ///
    /// ⚠️ **`decks.notes` comes back at the *end* of the column list here, where a real v42 file
    /// carries it twelfth**, because [`UNDO_V43`] can only `ADD COLUMN` and SQLite appends. That
    /// is harmless and was checked rather than assumed: the v43 rung **drops** the column, so its
    /// ordinal never reaches anything, and the stored `decks` text this fixture climbs to is
    /// byte-for-byte what [`USER_SCHEMA_SQL`] holds. Nothing here may start asserting a `decks`
    /// column *order*, which is the one thing the difference would show up in.
    ///
    /// **It takes no `foreign_keys` parameter**, for [`user_file_at_37`]'s reason — and here the
    /// pragma is worth a sentence rather than a shrug, since v43 is the first rung on this ladder
    /// with a `DROP TABLE` in its rewind: the two tables it drops are dropped child-first, and
    /// `deck_notes` has no dependant left once `deck_note_cards` is gone, so neither setting
    ...
    /// The deck whose group carries every case the v36 rung has to tell apart.
    const DECK_A: i64 = 1;
    ...
    ///
    /// **Head rewound past v38 and v37, then stamped with 35** — [`user_file_at_31`]'s
    /// construction one number line up, plus the two rewinds head has grown since. v36 writes
    /// no shape at all; it *moves rows*, so a v35 database and a v36 one **are** the same schema
    /// and renumbering is the whole of the difference between those two. There is no
    /// `UNDO_V36` for that reason — the same exemption v32 has, and one v33, v34, v35, v37, v38
    /// and v39 do not.
    ///
    ...
    /// open.
    ///
    /// **It read "head is a legitimate v35 shape — v36 writes none — so the construction did not
    /// have to change, only the stamp", and v38 ended that.** Three `decks` columns above it —
    /// v38's two and v39's one — mean head is no longer a v35 shape, so this fixture rewinds
    /// [`UNDO_V39`] and [`UNDO_V38`] like every other one
    /// on the ladder. Without them the climb meets its own `ALTER TABLE decks ADD COLUMN` over
    /// columns `create_user_schema` had already built and dies at `duplicate column name` —
    /// [`UNDO_V13`]'s loud failure, reaching a fixture that had been allowed to skip the rewind
    /// only because the rung above it wrote nothing.
    ///
    ...
    fn user_file_at_36() -> Connection {
        let conn = Connection::open_in_memory().unwrap();
        create_user_schema(&conn, "main").unwrap();
        // All three rewinds, because head is no longer a legitimate v35 shape three times over:
        // the token
        // rung adds `deck_tokens` and `decks.tokens_open`, so without `{UNDO_V37}` this fixture
        // hands `migrate_user` a table that already exists and v37 dies at
        // `table deck_tokens already exists`; the theory rung adds two `decks` columns, so
    ...
    ///
    /// **`condition` is stated and not left to the DEFAULT**, which it was until v35 landed: the
    /// grade is the grain's third term, the default moved from `'NM'` to `'NONE'`, and a row that
    /// took the new default would sit at a *different* grain from the `'NM'` row the rung is
    /// about to file — no conflict, no fold, and a test that quietly stopped testing the merge.
    fn user_file_at_36_with_a_matching_removed_row() -> Connection {
        let conn = user_file_at_36();
        conn.execute_batch(
            "INSERT INTO collection_entries
                 (card_id, set_code, collector_number, lang, finish, condition, quantity,
                  folder_id, created_at, updated_at)
             VALUES ('bolt-m10', 'm10', '146', 'en', 'nonfoil', 'NM', 2, 10, 0, 0);",
        )
        .unwrap();
        conn
    }
    ...
    /// terms.
    fn seed_v35_groups(conn: &Connection) {
        conn.execute_batch(
    ...
    // ---- v37: the tokens and emblems a deck brings ---------------------------------

    /// v37's table and the deck's flag exist on an UPGRADED file, the ladder is what put them
    ...
    #[test]
    fn v37_adds_deck_tokens_and_the_deck_flag() {
        let conn = user_file_at_36();
    ...
    #[test]
    fn v37_leaves_a_deck_that_predates_it_collapsed() {
        let conn = user_file_at_36();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
    ...
    #[test]
    fn v37_overrides_leave_with_the_deck_that_holds_them() {
        let conn = Connection::open_in_memory().unwrap();
        conn.pragma_update(None, "foreign_keys", "ON").unwrap();
        create_user_schema(&conn, "main").unwrap();
        conn.execute_batch(
    ...
    #[test]
    fn v37_refuses_a_duplicate_deck_and_oracle() {
        let conn = memory_pair();
        conn.execute_batch(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
                 VALUES (1, 'Krenko', 'commander', 0, 0);
    ...
    ///
    /// `the_v34_fixture_carries_none_of_v35`'s job one rung up, and without it every test above
    /// is the failure this repo has shipped before: a [`user_file_at_36`] that forgot to rewind
    /// would hand `migrate_user` a table that already exists, and the assertions would pass
    /// while watching nothing happen. The other order is loud on its own — a fixture that kept
    /// the table dies at `table deck_tokens already exists`, which is [`UNDO_V37`]'s whole
    /// reason — but loud is not the same as checked.
    #[test]
    fn the_v36_fixture_carries_none_of_v37() {
        ...
        let tokens: i64 = conn
            ...
            .unwrap();
        assert_eq!(tokens, 0, "the v37 fixture already carries `deck_tokens`");
        ...
        assert!(
            flag.is_err(),
            "the v37 fixture already carries `decks.tokens_open`"
        );
    }
    ...
    #[test]
    fn the_v36_rung_evicts_what_the_list_does_not_name() {
        let conn = user_file_at_36();
        assert_eq!(
            conn.query_row("PRAGMA main.user_version", [], |r| r.get::<_, i64>(0))
                .unwrap(),
            35,
            "the fixture is a v35 file"
        );
    ...
    #[test]
    fn the_v36_rung_splits_a_partly_claimed_row() {
        let conn = user_file_at_36();
        migrate_user(&conn).unwrap();
    ...
    #[test]
    fn the_v36_rung_reaches_every_deck() {
        let conn = user_file_at_36();
        migrate_user(&conn).unwrap();
    ...
    #[test]
    fn the_v36_rung_folds_into_a_row_recently_removed_already_holds() {
        let conn = user_file_at_36_with_a_matching_removed_row();
        migrate_user(&conn).unwrap();
    ...
    #[test]
    fn the_v36_rung_leaves_a_switched_off_pile_holding_its_cards() {
        let conn = user_file_at_36();
        migrate_user(&conn).unwrap();
        assert_eq!(
    ...
    #[test]
    fn the_v36_rung_takes_from_the_oldest_row_of_a_group_first() {
        let conn = user_file_at_36();
        migrate_user(&conn).unwrap();
    ...
    #[test]
    fn the_v36_rung_splits_the_trade_list_and_carries_the_story() {
    ...
    #[test]
    fn the_v36_rung_touches_no_folder_that_is_not_a_deck_group() {
        let conn = user_file_at_36();
        migrate_user(&conn).unwrap();
        let binder: i64 = conn
            .query_row(
    ...
    #[test]
    fn the_v36_rung_moves_copies_and_never_invents_or_loses_one() {
        let conn = user_file_at_36();
        let before: i64 = conn
    ...
    /// rung that errors blocks startup, and a hand-edited file without that folder must open.
    #[test]
    fn the_v36_rung_stamps_a_file_with_no_holding_area_rather_than_refusing_it() {
        let conn = user_file_at_36();
        ...
        // head, so it would go on passing over a rung that never ran. **The price of that choice
        // is this line**: it is head rather than this rung's own number, because `migrate_user`
        // climbs the whole ladder, so **every rung that lands owes these three assertions an
        // edit** — which is a cost the sentence above accepts rather than a defect. Paid four
        // times now: 35 → 36, 36 → 37 for the token rung, 37 → 38 for the theory-mark rung,
        ...
        // until fan-in. **Eight times**, and the count is the argument.
        //
    ...
    /// The fixture is a real v35 file and not head wearing a v35 label.
    ///
    /// The mirror of [`the_v34_fixture_carries_none_of_v35`], and it has to be read differently
    /// because v36 writes no shape: there is no column to probe for. What says the file sits
    /// **below** the rung is that the state the rung exists to clear is still there — a group
    /// holding a printing its deck does not list. Without that half every assertion above is
    /// vacuous.
    ///
    ...
    /// shape", and v38 ended that.** Three `decks` columns above v36 — v38's two and v39's one —
    /// mean head is no longer a v35 shape at all, so [`user_file_at_36`] rewinds [`UNDO_V39`]
    /// and [`UNDO_V38`] like every other fixture here —
    ...
    fn the_v35_fixture_carries_none_of_v36() {
        ...
        conn.execute(
            "INSERT INTO collection_entries
                (card_id,set_code,collector_number,lang,finish,condition,quantity,
                 created_at,updated_at)
             VALUES ('probe','lea','161','en','nonfoil','NONE',1,0,0)",
            [],
        )
        .expect("a v35 file accepts the sentinel the rung below this one introduced");
        let locked: i64 = conn
            .query_row(
                "SELECT count(*) FROM pragma_table_info('collection_folders')
                  WHERE name = 'locked'",
            ...
            .unwrap();
        assert_eq!(locked, 1, "a v35 file has everything v34 built");
    }

    /// **v38's two columns, and the deck that already existed when the rung ran.**
    ///
    ...
    /// and no group of older decks behaving differently for ever for a reason nothing on screen
    /// explains.
    ///
    /// **The deck is seeded before the climb rather than after it**, which is the whole of what
    /// this test is for. A row inserted at head takes the column default through the *frozen*
    /// [`USER_SCHEMA_SQL`] and would pass while the rung wrote `DEFAULT 0` — the one population
    /// that cannot be faked, read from the same side
    ...
    #[test]
    fn v38_gives_every_deck_both_theory_marks() {
        let conn = user_file_at_37();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
             VALUES (1, 'Burn', 'modern', 0, 0)",
            [],
        ...
        assert_eq!(
            (exact, named),
            (1, 1),
            "a deck that existed before the rung wears both marks after it"
        );
        ...
        let (exact, named): (i64, i64) = conn
            .query_row(
                "SELECT theory_mark_exact, theory_mark_name FROM decks WHERE id = 2",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
    ...
    /// already there.
    #[test]
    fn the_v38_rung_is_idempotent_over_an_already_upgraded_database() {
        let conn = user_file_at_37();
        migrate_user(&conn).unwrap();
        migrate_user(&conn).unwrap();
        let version: i64 = conn
    ...
    /// The fixture is a real v37 file and not head wearing a v37 label.
    ///
    ...
    /// column dies at `duplicate column name`, which is [`UNDO_V38`]'s whole reason.
    #[test]
    fn the_v37_fixture_carries_none_of_v38() {
        let conn = user_file_at_37();
        ...
        assert_eq!(
            ...
            0,
            "theory_mark_exact must not exist before the rung that adds it"
        );
        assert_eq!(
            has_column(&conn, "decks", "theory_mark_name"),
            0,
            "theory_mark_name must not exist before the rung that adds it"
        );
        ...
        assert_eq!(
            ...
            1,
            "a v37 file still has what v34 built"
        );
    }

    /// **v39's third column, and the deck that already existed when the rung ran.**
    ///
    /// `v38_gives_every_deck_both_theory_marks`'s job one rung up, and its argument unchanged:
    /// `NOT NULL DEFAULT 1` *is* the migration, so every deck on a reader's disk draws the red
    /// X from the first launch on the new build with no backfill and no group of older decks
    /// behaving differently for ever for a reason nothing on screen explains.
    ///
    /// **The deck is seeded before the climb rather than after it**, which is the whole of what
    /// this test is for. A row inserted at head takes the column default through the *frozen*
    /// [`USER_SCHEMA_SQL`] and would pass while the rung wrote `DEFAULT 0` — the one population
    /// a fresh worktree cannot be, because a fresh install never climbs a rung at all.
    #[test]
    fn v39_gives_every_deck_the_unplanned_mark() {
        let conn = user_file_at_38();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
             VALUES (1, 'Burn', 'modern', 0, 0)",
            [],
        ...
        assert_eq!(has_column(&conn, "decks", "theory_mark_unplanned"), 1);
        let unplanned: i64 = conn
            .query_row(
                "SELECT theory_mark_unplanned FROM decks WHERE id = 1",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(
            unplanned, 1,
            "a deck that existed before the rung wears the third mark after it"
        );
        ...
        let unplanned: i64 = conn
            .query_row(
                "SELECT theory_mark_unplanned FROM decks WHERE id = 2",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(unplanned, 1, "a deck is born with the third mark on");
    }

    /// The second pass has to be a no-op rather than `duplicate column name`.
    ///
    /// `the_v38_rung_is_idempotent_over_an_already_upgraded_database`'s job one rung up. SQLite
    /// offers no `IF NOT EXISTS` on `ALTER TABLE … ADD COLUMN`, so the version stamp is the only
    /// thing making a second launch survivable — and it is inside the rung's transaction with
    /// the `ALTER`, so a launch that died mid-rung left neither.
    #[test]
    fn the_v39_rung_is_idempotent_over_an_already_upgraded_database() {
        let conn = user_file_at_38();
        migrate_user(&conn).unwrap();
        migrate_user(&conn).unwrap();
        let version: i64 = conn
    ...
    /// The fixture is a real v38 file and not head wearing a v38 label.
    ///
    /// `the_v37_fixture_carries_none_of_v38`'s job one rung up. What it catches is the fixture
    /// rewound in only one of its two halves: the column really gone but the version left at
    /// head would leave `migrate_user` no rung to run, and every test above would pass while
    /// watching nothing happen. The other order is loud on its own — a fixture that kept the
    /// column dies at `duplicate column name`, which is [`UNDO_V39`]'s whole reason.
    ///
    /// **`theory_mark_name` is probed as *present*, and that is the half a version number
    /// cannot give you.** The rewind above this one is two rungs of `decks` columns stacked, and
    /// a chain that ran [`UNDO_V38`] as well would leave a file the stamp still calls 38 while
    /// it is really a 37 — which would then climb v38 and v39 together and pass every assertion
    /// about the third column while testing the wrong rung.
    #[test]
    fn the_v38_fixture_carries_none_of_v39() {
        let conn = user_file_at_38();
        ...
        assert_eq!(
            has_column(&conn, "decks", "theory_mark_unplanned"),
            0,
            "theory_mark_unplanned must not exist before the rung that adds it"
        );
        assert_eq!(
            has_column(&conn, "decks", "theory_mark_name"),
            1,
            "a v38 file still has both of v38's own columns"
        );
        assert_eq!(
            has_column(&conn, "decks", "theory_mark_exact"),
            1,
    ...
    /// **v40's column, and the deck that already existed when the rung ran.**
    ///
    /// `v39_gives_every_deck_the_unplanned_mark`'s job one rung up, with the default the *other*
    /// way round — and that reversal is the whole of what this test is for. `NOT NULL DEFAULT 0`
    /// is the rung: every deck on a reader's disk is a deck about cardboard, so the upgrade has
    /// to be invisible, and a rung that wrote `DEFAULT 1` would turn every deck in the database
    /// virtual at the first launch on the new build — no error, no empty state, just a
    /// collection that has quietly stopped knowing about any of them.
    ///
    /// **The deck is seeded before the climb rather than after it**, the rung below's argument
    /// verbatim: a row inserted at head takes the column default through the *frozen*
    /// [`USER_SCHEMA_SQL`] and would pass while the rung wrote `DEFAULT 1` — the one population
    /// a fresh worktree cannot be, because a fresh install never climbs a rung at all.
    #[test]
    fn v40_leaves_every_existing_deck_a_cardboard_deck() {
        let conn = user_file_at_39();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
             VALUES (1, 'Burn', 'modern', 0, 0)",
            [],
        ...
        assert_eq!(has_column(&conn, "decks", "virtual_only"), 1);
        let virtual_only: i64 = conn
            .query_row("SELECT virtual_only FROM decks WHERE id = 1", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(
            virtual_only, 0,
            "a deck that existed before the rung is still a deck about cardboard after it"
        );
        ...
        .unwrap();
        let virtual_only: i64 = conn
            .query_row("SELECT virtual_only FROM decks WHERE id = 2", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(virtual_only, 0, "a deck is born owning cardboard");
    }

    /// The second pass has to be a no-op rather than `duplicate column name`.
    ///
    /// `the_v39_rung_is_idempotent_over_an_already_upgraded_database`'s job one rung up. SQLite
    /// offers no `IF NOT EXISTS` on `ALTER TABLE … ADD COLUMN`, so the version stamp is the only
    /// thing making a second launch survivable — and it is inside the rung's transaction with
    /// the `ALTER`, so a launch that died mid-rung left neither.
    #[test]
    fn the_v40_rung_is_idempotent_over_an_already_upgraded_database() {
        let conn = user_file_at_39();
        migrate_user(&conn).unwrap();
        migrate_user(&conn).unwrap();
        let version: i64 = conn
    ...
    /// The fixture is a real v39 file and not head wearing a v39 label.
    ///
    /// `the_v38_fixture_carries_none_of_v39`'s job one rung up. What it catches is the fixture
    /// rewound in only one of its two halves: the column really gone but the version left at
    /// head would leave `migrate_user` no rung to run, and every test above would pass while
    /// watching nothing happen. The other order is loud on its own — a fixture that kept the
    /// column dies at `duplicate column name`, which is [`UNDO_V40`]'s whole reason.
    ///
    /// **`theory_mark_unplanned` is probed as *present*, and that is the half a version number
    /// cannot give you**, the test directly above reading `theory_mark_name` for its reason: the
    /// rewind above this one is three rungs of `decks` columns stacked, and a chain that ran
    /// [`UNDO_V39`] as well would leave a file the stamp still calls 39 while it is really a 38
    /// — which would then climb v39 and v40 together and pass every assertion about the kind
    /// while testing the wrong rung.
    #[test]
    fn the_v39_fixture_carries_none_of_v40() {
        let conn = user_file_at_39();
        ...
        assert_eq!(
            has_column(&conn, "decks", "virtual_only"),
            0,
            "virtual_only must not exist before the rung that adds it"
        );
        assert_eq!(
            has_column(&conn, "decks", "theory_mark_unplanned"),
            1,
            "a v39 file still has v39's own column"
        );
    }

    /// A v39 file walks all the way up keeping the deck it had, and the deck's other columns
    /// are the ones it went in with.
    ///
    /// **The rewind's round trip, which no single-column probe covers.** `user_file_at_39` is
    /// head with [`UNDO_V40`] run over it, so this is the only test that says the *pair* —
    /// `DROP COLUMN` down, `ADD COLUMN` up — lands back on a file the rest of the crate can
    /// read: every `decks` column `DECK_SELECT` names is still there, in an order the positional
    /// reads still agree with, with the seeded row's values intact.
    #[test]
    fn a_v39_file_round_trips_through_the_v40_rung() {
        let conn = user_file_at_39();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, theory_enabled, theory_mark_unplanned,
                                created_at, updated_at)
             VALUES (1, 'Burn', 'modern', 1, 0, 0, 0)",
            [],
        )
        .unwrap();
        ...
        let (name, theory, unplanned, virtual_only): (String, i64, i64, i64) = conn
            .query_row(
                "SELECT name, theory_enabled, theory_mark_unplanned, virtual_only
        ...
        assert_eq!(
            theory, 1,
            "the rung must not touch the other half of the pair"
        );
        assert_eq!(unplanned, 0, "nor the rung below it");
    ...
    /// **It is a cache, and [`SYNCED_TABLES`] does not name it.**
    ///
    ...
    /// The rung over a real v42 file: the column goes, the two tables arrive, and the grain
    /// fires.
    ///
    /// **The three assertions are three different rungs' worth of failure**, which is why they
    /// are in one test rather than three: v43 is the first step on this ladder that adds *and*
    /// removes, so "the tables are here" passing while "the column is gone" fails is a real
    /// half-migration rather than an impossible state.
    ///
    /// The grain is driven rather than read off `sqlite_master`, for
    /// `v37_refuses_a_duplicate_deck_and_oracle`'s reason: an index that exists and indexes the
    /// wrong pair of columns is a `CREATE` statement that looks right and refuses nothing, and
    /// the only question worth asking of a unique index is whether the second row bounces.
    #[test]
    fn v43_replaces_the_deck_notes_column_with_two_tables() {
        ...
        assert_eq!(
            ...
            0,
            "decks.notes survived the rung that drops it"
        );
        assert_eq!(has_column(&conn, "decks", "notes_open"), 1);
        ...
        // And the *other* half of the grain is a real column rather than a decoration: the same
        // card on a second note is a second fact and must land. Without this the test above
        // passes over an index grained on `oracle_id` alone.
        conn.execute_batch(
            "INSERT INTO deck_notes (deck_id, title, body, sort_order, created_at, updated_at)
                 VALUES (1, 't2', 'b', 1, 0, 0);
             INSERT INTO deck_note_cards (note_id, oracle_id, created_at, updated_at)
        ...
        .expect("one card may be named by two notes");
    }

    /// A deck delete reaches the notes and the notes reach their attachments.
    ///
    /// **Two cascades in one press, and the second is the one worth driving**: `deck_notes` hangs
    /// off `decks` and `deck_note_cards` hangs off `deck_notes`, so an attachment survives only
    /// if SQLite walks the chain — which it does not do at all unless `foreign_keys` is on, and
    /// that pragma is per-connection. Asserting the attachment rather than the note is what makes
    /// this a test of the chain instead of of its first link.
    #[test]
    fn a_deck_delete_cascades_to_notes_and_their_attachments() {
        ...
        .unwrap();
        let notes: i64 = conn
            .query_row("SELECT count(*) FROM deck_notes", [], |r| r.get(0))
            .unwrap();
        assert_eq!(notes, 0, "the note outlived its deck");
        let cards: i64 = conn
            .query_row("SELECT count(*) FROM deck_note_cards", [], |r| r.get(0))
            .unwrap();
        assert_eq!(cards, 0, "the attachment outlived its deck");
    }

    /// Every deck that predates the rung comes out of it with the Notes band collapsed.
    ///
    /// `v42_leaves_every_existing_deck_showing_its_stats`'s job one rung up, with the default the
    /// other way round — and *that* reversal is the whole of what this test is for. The stats
    /// band was on screen already, so v42 wrote `DEFAULT 1`; this band is new, so `DEFAULT 0`
    /// takes nothing from anybody. A rung that had copied its neighbour would open a section on
    /// every deck the reader owns, at the first launch on the new build, with nothing on screen
    /// saying why.
    ///
    /// **The deck is seeded before the climb rather than after it**, v40's and v42's argument
    /// verbatim: a row inserted at head takes the column default through the *frozen*
    /// [`USER_SCHEMA_SQL`] and would pass while the rung wrote `DEFAULT 1` — the one population a
    /// fresh worktree cannot be, because a fresh install never climbs a rung at all.
    #[test]
    fn v43_leaves_every_existing_deck_with_its_notes_band_shut() {
        let conn = user_file_at_42();
        conn.execute(
            "INSERT INTO decks (id, name, format_key, created_at, updated_at)
             VALUES (1, 'Burn', 'modern', 0, 0)",
        ...
        let open: i64 = conn
            .query_row("SELECT notes_open FROM decks WHERE id = 1", [], |r| {
                r.get(0)
            })
            .unwrap();
        assert_eq!(
            open, 0,
            "a deck that existed before the rung is not showing a band it has never had"
        );

        // **And the two disclosures beside it are untouched**, which is the half a matched set
        // of defaults could never show: `tokens_open` defaults 0, `stats_open` defaults 1 and
        // this one defaults 0, so an ALTER that landed on the wrong column changes exactly one
        // of these three numbers.
        let (tokens, stats): (i64, i64) = conn
            .query_row(
                "SELECT tokens_open, stats_open FROM decks WHERE id = 1",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        assert_eq!(
            (tokens, stats),
            (0, 1),
            "the three disclosures are three columns"
        );
    }

    /// The second pass has to be a no-op rather than `table deck_notes already exists`.
    ///
    /// `the_v42_rung_is_idempotent_over_an_already_upgraded_database`'s job one rung up, and it
    /// is owed **three** ways where that one was owed once: a bare `CREATE TABLE`, a bare
    /// `ADD COLUMN`, and — new on this ladder — a `DROP COLUMN` that would answer
    /// `no such column: notes` on a second pass. SQLite offers no `IF NOT EXISTS` on any of the
    /// three, so the version stamp is the only thing making a second launch survivable, and it
    /// is inside the rung's transaction with all of them.
    #[test]
    ...
    ///
    /// `the_v41_fixture_carries_none_of_v42`'s job one rung up. What it catches is the fixture
    /// rewound in only one of its halves: the tables really gone but the version left at head
    /// would leave `migrate_user` no rung to run, and every assertion above would pass while
    /// watching nothing happen.
    ///
    /// **`decks.notes` is probed as *present*, and that is the half neither a version number nor
    /// a table count can give you.** [`UNDO_V43`] has to put the column back as well as take two
    /// tables away, and a chain that dropped `notes_open` and forgot the `ADD COLUMN` would
    /// produce a fixture that still passes a "no `deck_notes` table" check, still says 42, and
    /// then dies mid-rung on `no such column: notes` — taking every unrelated test in the chain
    /// with it. **`stats_open` is probed as present too**, [`the_v41_fixture_carries_none_of_v42`]'s
    /// `collection_shares` line: a rewind that ran [`UNDO_V42`] as well would leave a file the
    /// stamp still calls 42 while it is really a 41.
    #[test]
    fn the_v42_fixture_carries_none_of_v43() {
        ...
        assert_eq!(
            has_column(&conn, "decks", "notes_open"),
            0,
            "notes_open must not exist before the rung that adds it"
        );
        assert_eq!(
            has_column(&conn, "decks", "notes"),
            1,
            "a v42 file still has the one paragraph v43 takes away"
        );
        assert_eq!(
            has_column(&conn, "decks", "stats_open"),
            1,
        ...
        let tables: i64 = conn
            .query_row(
                "SELECT count(*) FROM sqlite_master
                  WHERE type = 'table' AND name IN ('deck_notes', 'deck_note_cards')",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(tables, 0, "neither table may exist before the rung");
    }
    ...
    /// of the seven grain constants — [`DECK_CATEGORY_GRAIN`], [`DECK_LABEL_GRAIN`] and
    /// [`DECK_NOTE_CARD_GRAIN`] — are read by no SQL at all, and a constant nothing reads
    ...
    fn every_plain_grain_constant_names_the_index_the_head_schema_carries() {
        ...
        for (index, grain) in [
            ...
            // `ON CONFLICT`, so there is no second fence to fall back on. A constant with one
            // way to be fenced is fenced that way.
            ("idx_deck_note_cards_grain", DECK_NOTE_CARD_GRAIN),
        ] {
            let mut stmt = conn
