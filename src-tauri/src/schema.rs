...
pub(crate) mod tests {
    ...
    /// v35's sixth grade, taken back off — and the newest rewind on the user ladder.
    ///
    ...
    /// walks the ladder backwards. It was the top of it until [`UNDO_V35`] landed above it.
    ///
    /// **No index needs a line of its own**, [`UNDO_V20`]'s rule: the rung creates none, and
    ...
    /// one rung up. One rewind does it: v35 is the only rung above 34.
    ///
    ...
    fn user_file_at_35() -> Connection {
        let conn = Connection::open_in_memory().unwrap();
        create_user_schema(&conn, "main").unwrap();
        conn.execute_batch("PRAGMA main.user_version = 35;")
            .unwrap();
