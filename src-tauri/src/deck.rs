...
pub struct DeckPatch {
    ...
    /// Whether this deck draws the **green** theory mark — schema v36, and see
    ...
    /// Whether this deck draws the **blue** theory mark — schema v36, and
...
pub struct DeckRow {
    ...
    /// the plan named. Schema v36, `NOT NULL DEFAULT 1`, and per deck rather than per user for
...
fn deck_row(r: &rusqlite::Row<'_>) -> rusqlite::Result<DeckRow> {
    ...
    ///
    /// **It read `21` until schema v36 appended two columns**, which is the one number in this
...
pub fn duplicate_deck(conn: &Connection, id: i64) -> Result<DeckRow, String> {
    ...
    let copy: Option<(i64, String)> = tx
        .query_row(
            // **Schema v36's two marks are copied, not defaulted**, which is the same rule
...
mod tests {
    ...
    fn deck_row_json_uses_the_camel_case_names_the_frontend_expects() {
        let value = serde_json::to_value(DeckRow {
            ...
            notes: None,
            theory_enabled: true,
            // **Not both `true`, and not both `false`.** Schema v36 defaults both to 1, so a
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                // Schema v36's pair, deliberately disagreeing with each other: both default to
    ...
    /// A deck is born with both theory marks on, which is what makes schema v36 need no
