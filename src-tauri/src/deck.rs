...
///
/// **This module had a fourth, `deck_search_open`, and it left on 2026-09-07.** Whether the
/// editor's card search column is open is no longer a fact about decks: the collection and the
/// wishlist grew the same column, and one boolean row became [`crate::searchopen`]'s keyed map.
/// The old row is still on disk and still answers for the deck section, through that module's
/// read-side bridge rather than through anything here.
...
pub struct DeckPatch {
    ...
    /// Whether the editor's **Tokens & emblems** area is expanded — schema v37.
    ///
    ...
    /// Whether this deck keeps a **managed wishlist** — schema v48, and see
    ...
    /// Whether this deck draws the **red** theory mark — schema v39, and
    /// [`Self::theory_mark_exact`]'s rules throughout.
    ///
    /// **A third field for the same reason there are two above it**, one tier further out: the
...
pub struct DeckRow {
    ...
    /// every write that changes the deck (schema v48, `NOT NULL DEFAULT 1`, issue #512).
    ///
...
static DECK_SELECT: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
    format!(
        "SELECT d.id, d.name, d.format_key, fs.display_name, d.description,
            ...
            d.virtual_only, d.stats_open, d.notes_open, d.token_stack, d.managed_wishlist,
            {images}
       FROM decks d
...
fn deck_row(r: &rusqlite::Row<'_>) -> rusqlite::Result<DeckRow> {
    ...
    ///
    /// User schema v48 moved it to 29, appending `managed_wishlist`.
    ...
    Ok(DeckRow {
        ...
        // 28, at the end of the named list, same rule — schema v48's switch, appended after
        // v47's `token_stack`, never inserted.
        managed_wishlist: r.get(28)?,
...
struct DeckBefore {
    ...
    virtual_only: bool,
    /// Schema v48's managed-wishlist switch, for the history row.
    ...
    let changed = tx
        .execute(
            "UPDATE decks SET
                ...
                managed_wishlist = coalesce(?22, managed_wishlist),
...
fn record_deck_edit(
    ...
    // `theoryMarkExact`, `theoryMarkName` and `theoryMarkUnplanned`, camelCase — `xGroup`'s
    // rule, and these are the
    // third, fourth and fifth multi-word keys in this function. The `default` arm of `auditText.ts`'s
    ...
    //
    // **Three arms rather than one**, which is the schema's own three-columns-not-one argument
    // read
    ...
    // into either of them. Three rows also make the drawer read the way every other set of
    ...
    if let Some(to) = patch
        ...
        .filter(|m| *m != before.theory_mark_unplanned)
    {
        field(
            "theoryMarkUnplanned",
    ...
    let copy: Option<(i64, String, bool)> = tx
        .query_row(
            ...
            "INSERT INTO decks (name, format_key, description, cover_kind, cover_card_id,
                                ...
                                theory_mark_unplanned, virtual_only, token_stack, managed_wishlist,
             ...
             SELECT name || ' (copy)', format_key, description, cover_kind, cover_card_id,
                    ...
                    virtual_only, token_stack, managed_wishlist,
...
/// wishlist** put every card at the root until then and offered no choice at all; the root is
/// still what a caller that names nothing gets, so nothing written before this argument existed
/// has changed its meaning. The id is passed into [`crate::wishlist::WishInput::folder_id`] and
/// this function decides nothing else about it.
///
/// **Naming a folder writes a *second* wish rather than moving the first**, which is
/// [`WISHLIST_GRAIN`](crate::schema::WISHLIST_GRAIN)'s fourth term — `coalesce(folder_id, 0)` —
/// earning its place. A card the reader filed into *Ordered* last week and the same card sent to
/// the root today are two rows on that grain, and `add_wish`'s `DO UPDATE` clause touches
/// neither one's folder. Without the term this press would land on the row they had already
/// filed and raise its quantity, so the wish would appear to **move** as a side effect of
...
/// a deck's kind is a fact about the argument and needs no database open ([`update_deck`]'s rule
/// for its validations), while a folder's existence is a fact about a table this very
/// transaction is about to write against.
///
...
pub fn missing_to_wishlist(
    ...
    // kind above it: that refusal reads an argument and this one reads a table the loop below is
    // about to write against, so it has to see what that write will see.
    ...
    for (oracle_id, (name, quantity)) in missing {
        ...
        crate::wishlist::add_wish_silent(
            ...
            &crate::wishlist::WishInput {
                ...
                // nowhere, which is what every press meant before 2026-09-09. It is a grain
                // term, so naming a folder adds a wish rather than moving one; the argument is
...
mod tests {
    ...
    fn deck_row_json_uses_the_camel_case_names_the_frontend_expects() {
        let value = serde_json::to_value(DeckRow {
            ...
            // three times or none at all — the rule the three comments in the expectation below
            // already state about `defaultCategoryId`, `gameKey` and `bracket`, applied to the
            // one set of fields here that could be confused with each other.
            //
            // **Three booleans cannot all disagree**, so this fixture cannot catch every swap
            // by value alone and does not pretend to: `exact` and `unplanned` read the same,
            // and what separates *them* is the key each is asserted under below. The values are
            // what catch a field that never reached the wire; the names are what catch a pair
            // that crossed.
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                // set of positional reads could swap without changing a single value's type.
                // The **keys** are what separate `exact` from `unplanned`, which share a value
                ...
                "bracket": 3,
                // Schema v37, and `tokensOpen` rather than `tokens_open`: the panel reads this
    ...
    /// A deck is born with all three theory marks on, which is what makes schema v38 and v39
    /// need no backfill: the columns' own `DEFAULT 1`, never a Rust fallback.
    ///
    /// **`create_deck` names none of the three columns in its INSERT and must not start to**,
    /// which is
    /// the sentence this asserts from the outside. `theory_enabled`'s absence is resolved in
    /// Rust because [`DeckInput`] can carry it; these three are not on that struct at all, so
    /// the
    /// DDL is the only thing that decides, and a Rust default beside it would be a second
    /// answer to one question.
    #[test]
    fn a_new_deck_draws_every_theory_mark() {
        let conn = seeded();
        let deck = create_deck(&conn, &input("Burn", "modern")).unwrap();
        assert!(
            deck.theory_mark_exact && deck.theory_mark_name && deck.theory_mark_unplanned,
            "on for a new deck — the columns' own DEFAULT 1"
        );
        let read = read_deck(&conn, deck.id).unwrap().unwrap();
        assert!(
            read.theory_mark_exact && read.theory_mark_name && read.theory_mark_unplanned,
            "…including through `DECK_SELECT`'s positional reads, which is where a column \
             added anywhere but the end goes wrong silently"
        );
    }

    /// All three switches survive a write and a read, **independently**.
    ///
    /// A deck can draw any subset of the three marks, so all eight states are written and
    /// read back: the `coalesce(?n, column)` contract means a mis-numbered `?` hole writes over
    /// its neighbour, and three adjacent `INTEGER` columns holding a `bool` are exactly the set
    /// where that failure has nothing to make it visible. `(true, true, true)` is deliberately
    /// not
    /// first — it is the state a new deck is already in, so leading with it would let a set of
    /// holes that write nothing at all pass the first round.
    ///
    /// **The last state is all `false`** and it is left standing, which is the one a
    /// single ladder-shaped column could not have spelled: a reader who wants no theory mark at
    /// all has said something, and it is not the same
    /// as switching the theory list off.
    #[test]
    fn every_theory_mark_round_trips_independently() {
        let conn = seeded();
        let deck = create_deck(&conn, &input("Burn", "modern")).unwrap();
        for (exact, named, unplanned) in [
            (false, true, true),
            (true, false, true),
            (true, true, false),
            (false, false, true),
            ...
            let row = update_deck(
                ...
                &DeckPatch {
                    ...
                    theory_mark_unplanned: Some(unplanned),
                    ..Default::default()
                },
            )
            .unwrap();
            assert_eq!(
                (
                    row.theory_mark_exact,
                    row.theory_mark_name,
                    row.theory_mark_unplanned
                ),
                (exact, named, unplanned),
                "the readback is the write"
            );
            let read = read_deck(&conn, deck.id).unwrap().unwrap();
            assert_eq!(
                (
                    read.theory_mark_exact,
                    read.theory_mark_name,
                    read.theory_mark_unplanned
                ),
                (exact, named, unplanned),
                "…including through `DECK_SELECT`'s positional reads"
            );
        ...
        // must leave the other two exactly where they were, and with all three bound in the
        // loop above a set of holes crossed over each other would have passed every round.
        ...
        assert!(
            !only_exact.theory_mark_unplanned,
            "…and so was the third, which is the one `?18` would have reached"
        );

        // And the newest field alone, which is the half the two lines above cannot cover: a
        // patch naming *only* `?18` is what catches a hole bound one short of the end.
        let only_unplanned = update_deck(
            &conn,
            deck.id,
            &DeckPatch {
                theory_mark_unplanned: Some(true),
                ..Default::default()
            },
        )
        .unwrap();
        assert!(only_unplanned.theory_mark_unplanned);
        assert!(
            only_unplanned.theory_mark_exact && !only_unplanned.theory_mark_name,
            "the other two are exactly where the previous patch left them"
        );
        ...
        // named here are the ones a hole one out would actually reach: `?16`, `?17` and `?18`
        // are the last three in the list, and `game_key` and `bracket` are `?13` and `?14`.
        let after = update_deck(&conn, deck.id, &DeckPatch::default()).unwrap();
        assert!(after.theory_mark_exact && !after.theory_mark_name && after.theory_mark_unplanned);
        assert_eq!(after.name, "Burn");
        assert_eq!(after.game_key, DEFAULT_GAME);
    ...
    ///
    /// `the_x_group_switch_round_trips_and_is_recorded_once`'s job for the trio, with the one
    /// thing that test could not have: **all three fields move in one Save**, which is how the
    /// deck
    /// settings dialog actually sends them, and that press must land as **three** rows. One row
    /// saying "changed the theory marks" could be worded into none of the three decisions —
    /// which is
    /// `auditText.ts`'s problem and the reason the keys are three.
    ///
    /// The keys are asserted as literal `theoryMarkExact` / `theoryMarkName` /
    /// `theoryMarkUnplanned` strings, `xGroup`'s
    ...
    fn each_theory_mark_records_its_own_history_row_once() {
        ...
        // One Save moving all three, which is three rows and not one.
        update_deck(
            &conn,
            deck.id,
            &DeckPatch {
                ...
                theory_mark_unplanned: Some(false),
                ..Default::default()
            },
        )
        .unwrap();
        ...
        .unwrap();
        // The newest field alone, same shape — the row that says v39's key reaches the drawer
        // under its own name rather than under one of its neighbours'.
        update_deck(
            &conn,
            deck.id,
            &DeckPatch {
        ...
        let words: Vec<serde_json::Value> = crate::deck_audit::list(&conn, deck.id, 20)
            ...
            .filter(|p: &serde_json::Value| {
                p["field"] == "theoryMarkExact"
                    || p["field"] == "theoryMarkName"
                    || p["field"] == "theoryMarkUnplanned"
            })
            .collect();
        assert_eq!(
            words,
            vec![
                json!({ "field": "theoryMarkUnplanned", "from": false, "to": true }),
                json!({ "field": "theoryMarkName", "from": false, "to": true }),
                json!({ "field": "theoryMarkUnplanned", "from": true, "to": false }),
                json!({ "field": "theoryMarkName", "from": true, "to": false }),
                json!({ "field": "theoryMarkExact", "from": true, "to": false }),
            ],
            "newest first: one Save moving all three is three rows, the repeat is none, \
             and each single-field Save names only its own field"
        );
    }

    /// Ctrl+Z puts all three marks back, which is what putting them on
    /// [`crate::deck_undo`]'s `DECK_FIELDS` buys.
    ///
    /// **All three moved in one press, and all three come back.** A list carrying only two
    /// would
    /// restore part of one Save — worse than restoring none of it, because the drawer would
    /// still name the change it had not undone.
    #[test]
    fn undo_puts_every_theory_mark_back() {
        let conn = seeded();
        let deck = create_deck(&conn, &input("Burn", "modern")).unwrap();
        update_deck(
            &conn,
        ...
        assert_eq!(
            (
                moved.theory_mark_exact,
                moved.theory_mark_name,
                moved.theory_mark_unplanned
            ),
            (false, false, false)
        );
        ...
        assert_eq!(
            (
                back.theory_mark_exact,
                back.theory_mark_name,
                back.theory_mark_unplanned
            ),
            (true, true, true),
            "one press moved all three, so one Ctrl+Z has to move all three back"
        );
    }

    /// A copy inherits all three marks, `separate_x_group`'s and `bracket`'s rule and not the
    /// three
    /// `last_*` columns': these are answers *about the deck*, not about how the reader was
    /// looking at it a moment ago.
    ///
    /// **All three switched off on the source, which is the only setting that can fail.** The
    /// columns are `DEFAULT 1`, so a `duplicate_deck` that named none of them would answer
    /// `true`
    /// for all three and a test written over an all-on source would pass against exactly the
    /// bug it
    /// was for.
    #[test]
    fn a_duplicate_inherits_every_theory_mark() {
        let conn = seeded();
        let deck = create_deck(&conn, &input("Burn", "modern")).unwrap();
        update_deck(
            &conn,
        ...
        assert_eq!(
            (
                copy.theory_mark_exact,
                copy.theory_mark_name,
                copy.theory_mark_unplanned
            ),
            (false, false, false),
    ...
    ///
    /// **Both presses pass `folder_id: None`, and the fold is why that matters.** The root is
    /// [`WISHLIST_GRAIN`](crate::schema::WISHLIST_GRAIN)'s fourth term like any other value, so
    /// two presses land on one row only because they name the same destination — not because
    /// this command has no destination to name, which is what it had until 2026-09-09. The two
    /// destinations kept apart are
    /// `the_same_card_at_the_root_and_in_a_folder_is_two_wishes`.
    #[test]
    fn missing_to_wishlist_writes_any_printing_wishes_through_the_wishlist_grain() {
        ...
        assert_eq!(missing_to_wishlist(&conn, deck.id, None).unwrap(), 1);
    ...
    // ---------------------------------------------------------------------------------------
    // Where the shopping list is filed. **Send missing to wishlist** put every card at the
    // wishlist's root until 2026-09-09 and offered no choice at all
    // ([issue #437](https://github.com/Msgaihede/mtg-grimoire/issues/437)). What makes offering
    // one safe is `WISHLIST_GRAIN`'s **fourth** term: `coalesce(folder_id, 0)` is in the
    // conflict target, so naming a folder adds a wish rather than quietly moving the one the
    // reader filed last week.
    // ---------------------------------------------------------------------------------------

    /// One folder at the root of the **wishlist's** tree. This file already builds deck folders
    /// and collection folders, so the name says which of the three cabinets is meant.
    ...
    /// Every wish as `(oracle card, folder, copies)` in write order — this command writes an
    /// any-printing wish, so the oracle id is the one identifier every row of it carries. The
    /// folder is [`WISHLIST_GRAIN`](crate::schema::WISHLIST_GRAIN)'s fourth term, read as an
    /// `Option<i64>` so that the root is a `None` rather than a magic number and a row which
    /// quietly acquired a folder fails the comparison instead of passing a count.
    fn filed_wishes(conn: &Connection) -> Vec<(Option<String>, Option<i64>, i64)> {
        conn.prepare(
            "SELECT oracle_id, folder_id, quantity
    ...
    /// **The wishes land in the folder the press named**, which is the whole of what picking one
    /// does: the id rides on every [`crate::wishlist::WishInput`] the loop builds. Two cards
    /// short rather than one, so a build that carried the folder on the first wish and lost it
    /// afterwards has somewhere to fail.
    #[test]
    fn missing_to_wishlist_files_the_wishes_in_the_folder_it_was_given() {
        ...
        assert_eq!(
            filed_wishes(&conn),
            vec![
                (Some("o1".to_owned()), Some(ordered), 4),
                (Some("o2".to_owned()), Some(ordered), 2),
            ],
    ...
    /// is the situation the check is for: another window emptied the cabinet between the menu
    ...
    fn a_folder_that_is_gone_is_refused_before_a_wish_is_written() {
        ...
        assert_eq!(
            missing_to_wishlist(&conn, settled, None).unwrap(),
            0,
        ...
        assert_eq!(
            missing_to_wishlist(&conn, short, Some(gone)).unwrap_err(),
            crate::deck_meta::FOLDER_GONE
        );
        assert_eq!(
            missing_to_wishlist(&conn, settled, Some(gone)).unwrap_err(),
            crate::deck_meta::FOLDER_GONE,
    ...
    /// the first one wrote and raises it to eight — the reader watching four copies they had
    ...
    /// deck changes between them — a wish moves no cardboard — so the shortfall is re-read to
    /// the same four copies of the same oracle card, and the grain's other three terms all
    /// match. Two rows can therefore only have been separated by the fourth; a fixture that
    /// varied the quantity would have passed with that term gone.
    ///
    ...
    fn the_same_card_at_the_root_and_in_a_folder_is_two_wishes() {
        ...
        assert_eq!(
            filed_wishes(&conn),
            vec![
                (Some("o1".to_owned()), None, 4),
                (Some("o1".to_owned()), Some(ordered), 4),
            ],
            "two rows of four copies, not one row of eight"
        );
    ...
    fn missing_to_wishlist_refuses_a_virtual_deck() {
        ...
        let err = missing_to_wishlist(&conn, tracked, None).unwrap_err();
        ...
        assert_eq!(
            missing_to_wishlist(&conn, cardboard, None).unwrap(),
            1,
