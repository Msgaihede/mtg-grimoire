    ...
    let tx = conn.transaction()?;
    {
        ...
        for c in batch.iter() {
            stmt.execute(params![
                ...
                c.search_text,
                c.raw,
                // `?44`, matching the column list in `STAGING_INSERT` — the two lists are one
                // statement written twice and must move together. Corpus schema 3.
                c.produced_mana,
                // `?45`, last. Corpus schema 5, and the one column here whose *format* is a
