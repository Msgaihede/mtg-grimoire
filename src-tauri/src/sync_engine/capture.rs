...
pub const TABLES: [Spec; 16] = [
    ...
    Spec {
        ...
        fields: &[
            ...
            // Schema v49's mode, which replaced v48's `managed_wishlist` switch under a **new
            // name** on purpose: an older peer skips an unknown field, where a word landing in its
            // INTEGER column would fail that device's deck read. The mode syncs, because it is the
