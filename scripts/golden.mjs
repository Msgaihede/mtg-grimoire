...
/**
 * The **registry** golden — one level up from the rendered bytes, and it closes a hole the
 * `.txt` files structurally cannot.
 *
 * `write_line` renders exactly seven ids; every other id is invisible in the six line formats.
 * So adding `lang` to `plain`'s `optional` on one side only moves **zero golden bytes** — while
 * changing the *fold key*, which needs two corpus rows differing solely in that field to show
 * up, and none exist. One printing in two languages would then export as two lines from the
 * mirror and one line from the dialog, with all 70 files and both suites green. Spec §6's
 * "skipping the Rust half is a red `cargo test`" was true of CSV alone, whose header row **is**
 * the field list spelled out.
 *
 * So the field lists themselves are committed, in the same shape and read by the same two
 * suites: for every (format, surface) pair, what `availableFields` and `defaultFields` answer.
 * `SURFACE_FIELDS` is written out beside them because it is the one table whose whole content
 * is not recoverable from the 21 intersections — a field no format offers would drop out of
 * every one of them.
 */
writeFileSync(
  ...
  `${JSON.stringify(
    {
      note: "Generated by scripts/golden.mjs. Asserted by golden.test.ts and fields.rs.",
      surfaces: Object.fromEntries(
        Object.entries(SURFACE_FIELDS).map(([surface, ids]) => [surface, [...ids]]),
      ),
      available: Object.fromEntries(
        EXPORT_FORMATS.flatMap((format) =>
          Object.keys(SURFACE_FIELDS).map((surface) => [
            `${format}.${surface}`,
            availableFields(format, surface),
          ]),
        ),
      ),
...
console.log(`wrote ${written} golden files and fields.json`);
