...
| [external-links.md](docs/reference/external-links.md) | The `Open on …` ladder — the TCGplayer product ids Scryfall has always stored measured across the corpus, the two-word printing vocabulary verified in a browser, the decision table's seven rows and the `soleFinish` near-duplicate that must not be collapsed, and why tcgcsv.com was measured and refused |
