...
export interface DeckInput {
   ...
   *
   * **At create this sets the column and seeds nothing**, because a deck being born has no live
   * cards to seed from. Contrast {@link DeckPatch.theoryEnabled}, whose documented behaviour is
   * that switching it on copies the live list across — there is nothing here for that to copy.
   *
   * Worth knowing one step further out: the patch seeds on the **transition** off → on, so a
   * deck born with theory already on has made that transition at birth and no later patch will
   * ever seed it. Filling the plan from a live list built up afterwards is
   * {@link ipc.deckTheoryCopyFromLive}, which is the reader's button for exactly that.
   */
