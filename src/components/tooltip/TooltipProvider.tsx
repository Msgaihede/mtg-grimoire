    ...
    return {
      enter(anchor: HTMLElement, content: ReactNode, options: TooltipOptions) {
        ...
        timers.current.open = window.setTimeout(() => {
          ...
          // tooltip *for* any more. The docked pane's `PrintingPreview` took the same guard on
          // its own dwell timer for the same reason, until it was deleted on 2026-09-03:
          // measuring a detached node answers all zeros, and a hint that opens at the window's
          // corner attached to nothing is worse than one that never opens.
