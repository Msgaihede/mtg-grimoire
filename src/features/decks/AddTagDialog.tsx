/**
 * Put a label on the card the reader right-clicked: pick one of the tags this list is not
 * already using, or make one that does not exist yet.
 *
 * ## What it replaced, and why the shape changed twice
 *
 * It was `NewTagDialog`, which was itself a replacement for a text field inside the context
 * menu. The field made a tag in {@link DEFAULT_TAG_COLOR} because a menu has no room for a
 * colour picker; that stopped being right when a tag's colour became the reader's own, since
 * every label born from a menu would be gold and have to be visited again to be told apart.
 * So the fast path became the radio rows above it — the deck's own labels, one press each —
 * and making a *new* one became a deliberate act in a dialog.
 *
 * Schema v21 changed what "the deck's own labels" means. A tag belongs to no deck now, and the
 * rows above are **the tags this list is wearing** — which is the fast path the issue asks for
 * and is genuinely faster than it was, because a deck's list no longer fills up with labels it
 * has used once. The cost is that every *other* tag the reader has ever made is off the menu,
 ...
 * ## One field, two jobs
 *
 * Typing narrows the list **and** is the name a new tag would get. That is not a trick: they
 * are the same question. A reader who types "cut" and sees "Cut candidate" wanted the tag they
 ...
 * once, which is what the duplicate guard is for: a name an existing tag already holds refuses
 ...
 *
 * The comparison is `tagNames.ts`' and not `===`: `removal` is `Removal`, and a `Café` typed
 ...
 * replaced already followed, for the reason written out in full on `DeckCardMenuDeps.addTag`:
...
import { TagColorPanel, TagSwatch } from "./TagColorPicker";
...
export interface AddTagDialogProps {
  ...
  /** Every tag **not already in this list**, most-used first — the app-wide list minus the
   *  rows the context menu already offered. The editor narrows it, because the editor is what
   *  knows both halves. */
  choices: readonly GlobalTag[];
  /** The editor's `deck_tag_create` in flight, which is the one thing this body cannot know
   *  for itself — the mutation is mounted a floor up so it can outlive this dialog. */
  pending: boolean;
  /** Put an existing tag on the card. One press, and the dialog closes. */
...
/**
 * `26rem`, the width the create-only dialog had: the list below the field is one line per tag
...
export function AddTagDialog({
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
    >
      <AddTagBody choices={choices} pending={pending} onPick={onPick} onCreate={onCreate} />
    </Dialog>
  );
}
...
function AddTagBody({
  ...
  const shown = useMemo(() => {
    ...
    return choices.filter((t) => tagNameKey(t.name).includes(key));
  }, [choices, trimmed]);

  // A name one of the **choices** holds is a row on screen to point at. One held by a tag that
  ...
  const clash = findTagByName(choices, trimmed);
  ...
  return (
    ...
    >
      <div>
        <label htmlFor="add-tag-dialog-name" className="sr-only">
          Find or name a tag
        </label>
        <input
          ref={ref}
          id="add-tag-dialog-name"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Find or name a tag…"
            ...
            <li key={tag.id}>
              ...
              >
                ...
                {/* What makes this list worth reading top-down: the reach the order is by.
                    A tag nothing wears says nothing rather than "0 cards", which would be a
      ...
      {shown.length === 0 && (
        <p className="text-[0.6875rem] text-dim">
          {choices.length === 0
            ? "Every tag you have is already in this list. Name a new one above."
            : `No other tag matches “${trimmed}”. Name it below to make it.`}
        </p>
      )}
      ...
      {/* The picker is open rather than behind a press, for the reason the create-only dialog
          ...
          above it, because picking an existing tag is the commoner act and a colour is a
      ...
      <div className="flex justify-end">
        <button type="submit" disabled={pending || !canCreate} className={META_SUBMIT}>
          {trimmed === "" ? "Create tag" : `Create “${trimmed}”`}
        </button>
      </div>
    </form>
  );
