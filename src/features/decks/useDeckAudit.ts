 ...
 *  array on every render of a drawer that is still waiting. */
const NONE: readonly DeckAuditEntry[] = [];

/**
 * How much history the drawer asks for: the backend's own ceiling, `deck_audit::MAX_LIMIT`.
 *
 * **A cap, not a page.** This table grows by one row per edit, so a deck a person has actually
 * built is hundreds of rows rather than millions, and the drawer shows the most recent day or
...
/**
 * One deck's history, grouped into the day sections the drawer draws.
 *
 ...
 * refreshes the drawer sitting beside it. That is why no mutation anywhere needs to know this
 * key exists.
 *
 * **No variant.** The history covers both lists — a Theory edit is history too — and each entry
 * carries its own {@link DeckAuditEntry.variant}. A drawer that filtered would be hiding half
...
/** The whole of what a history drawer consumes, named so the view and the hook agree. */
