...
/**
 * An undo is a deck write, so it makes the same root wrong as the write it reverses —
 * `["decks"]`, and nothing wider: nothing here touches `collection_entries`, so the collection,
 * the search wall and the wishlist are provably unmoved.
 */
describe("useDeckUndo invalidation", () => {
  it("marks only the decks stale when a change is undone", async () => {
    seedOwned(client);
