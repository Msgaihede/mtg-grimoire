...
/**
 * Every label the reader owns that this deck's list is **not** already wearing — the editor's
...
function mount(props: Partial<Parameters<typeof AddLabelDialog>[0]> = {}) {
  const onPick = vi.fn();
...
describe("AddLabelDialog", () => {
   ...
   */
  it("offers every other label, most-used first, and says how far each reaches", async () => {
    mount();
    const rows = await screen.findAllByRole("listitem");
    expect(rows.map((li) => li.textContent)).toEqual([
      "Cut candidate12",
      "Budget swap5",
      // A label nothing wears prints no number: "0" would be arithmetic about a label that has
      // simply never been used.
      "Combo piece",
    ]);
  });

  it("puts an existing label on the card in one press, without creating anything", async () => {
   ...
   * the label they meant wanted that label; one who types "cut" and sees nothing wanted a new one.
   * Narrowed on `labelNameKey`, so a substring matches whatever capitals or accent form either
   * side was typed in.
   */
  it("narrows the list as the reader types, ignoring case", async () => {
    ...
    await user.type(screen.getByLabelText("Find or name a label"), "CANDID");
   ...
   * refuses the name and is the authority — one row per name is a table property. But the
   * label is on screen, so pressing Create and waiting for a refusal would be the app knowing
   * the answer and declining to say so.
   */
  it("refuses to create a name one of the choices already holds, and points at the row", async () => {
    const { onCreate } = mount();
    const user = userEvent.setup();

    await user.type(screen.getByLabelText("Find or name a label"), "  budget SWAP ");
  ...
  /** Nothing to offer is its own sentence: "no other label matches" would be about a filter the
