...
/**
 * **The game-changer spotlight**: the deck's own answer to *which of these cards are the powerful
 * ones*, drawn by fading everything that is not one.
 *
 * The ledger's `6 game changers` chip is the control (`DeckLedger`) and the deck's view box is
 * what wears the state (`DeckEditor`); what is here is the vocabulary the four views spread onto
 * their cards so that the one CSS rule in `src/index.css` can find them. Nothing in this module
 * decides *when* the spotlight is on — it is hover or a latch, composed by the editor — and no
 * view has to know it exists beyond spreading {@link deckCardDimmed} where it already spreads
 * {@link deckCardSelectedProps}.
 *
 * ## Why the card wears a **class** and not an attribute
 *
 * Every other mark here is an attribute — {@link SELECTED_ATTR}, {@link LANDED_ATTR},
 * {@link DECK_GROUP_ATTR} — and every one of them exists so that something can *find* a card
 * after the fact: a test, `scripts/cdp.mjs --probe` in the shipped window, the editor handing the
 * caret back to a pile. Each is a **question the DOM can answer**. This is not one. It is read by
 * a single rule in `src/index.css` and by nobody else at all, and what a stylesheet addresses is
 * a class — so an attribute here would be a query handle for a question that is never asked.
 *
 * **The table is what settles it in practice**, and not because it could not take an attribute:
 * `DeckTableRow` owns the element it spreads `RowRenderProps` onto and already stamps
 * {@link deckCardSelectedProps} there, so a fifth spread was available. What it has *instead* is
 * `rowClassName`, `VirtualTable`'s existing hook for exactly "the caller wants a class on this
 * row", sitting beside `isSelected` — so the class route is the one that reaches all four views
 * through machinery each of them already has, with no view growing a seam and the shared table
 * primitive growing nothing at all. `TableView`'s own comments record refusing to push a handler
 * into that primitive for one caller; this is the same rule met by not needing to.
 *
 * ## Why the card marks the **dimmed** state rather than the lit one
 *
 * Because the mark is on the *majority* of the cards and does nothing at all until an ancestor
 * arms it, which is what keeps the stylesheet's side of this a single flat descendant selector.
 * Marking the game changers instead would make the rule `[…] *:not(.deck-gc-lit)` — a `:not()`
 * over a broad subject, evaluated against every element under the deck. This repo has measured
 * what that costs: a `:has()` on a broad subject took one jsdom play from **3.5 s to 15 s** and
 * the whole run from 181 s to 231 s. The cheap selector is the design, not an accident of which
 * boolean was to hand.
 */
export const GC_DIMMED = "deck-gc-dimmed";

/**
 * What a deck card spreads into its class list so the game-changer spotlight can find it.
 *
 * `undefined` for a game changer, which is the card that stays lit — and `undefined` rather than
 * `""` so `cn` drops it entirely and a lit card's class list is byte-for-byte what it was before
 * this existed. `gameChanger` is `DeckCard.gameChanger`, which is `boolean | null`: **`null` is
 * an orphan or a printing the format has no opinion about, and it dims**, exactly as
 * `deckCardName` and `CardMarks` already treat anything but `true` as *not a game changer*.
 */
export function deckCardDimmed(gameChanger: boolean | null): string | undefined {
  return gameChanger === true ? undefined : GC_DIMMED;
}

/**
 * The attribute the deck's view container carries while the spotlight is on.
 *
 * Present-or-absent for {@link SELECTED_ATTR}'s reason — the stylesheet queries
 * `[data-gc-spotlight]` and needs no value — and it is on a *container* rather than on each card
 * because the state is one fact about the deck rather than N facts about cards. The editor arms
 * the nearest common ancestor of the four views, which is deliberately not the box that also
 * holds the docked search column: those tiles are not in the deck and must never fade.
 */
export const GC_SPOTLIGHT_ATTR = "data-gc-spotlight";

/** What the container that holds the four views spreads to arm the spotlight — and nothing at
 *  all when it is off, so a deck nobody is spotlighting grows no attribute. */
export function deckSpotlightProps(on: boolean) {
  return on ? { [GC_SPOTLIGHT_ATTR]: "" } : {};
}
