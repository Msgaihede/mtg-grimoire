...
describe("GridView tiles", () => {
  ...
  it("draws one crowned count on both card faces, and spells the fact out on neither", () => {
    ...
    for (const element of [
      <GridView key="grid" groups={groups} marketplace={TCG} />,
      <StackView key="stack" groups={groups} marketplace={TCG} />,
    ]) {
      render(element);
...
describe("TextView lines", () => {
  ...
  it("opens every line with the crown's gutter, and gilds the count on a game changer", () => {
    render(<TextView groups={GROUPS} marketplace={TCG} violations={VIOLATIONS} />);
