 ...
 * rail fits beside the columns — and the rail's own width. A second spelling of "300px" would let
 * the rail wrap at a width the columns had stopped agreeing with, and the layout would be wrong
  ...
  return (
    ...
    >
      {/* The flowing columns, one column wide at minimum — and that `minWidth` is what decides,
          ...
          **There is no `maxWidth` on it, and the rail is at the right edge because this box takes
          everything else** (changed 2026-08-18, and `StackView`'s change arriving here as the cap
          itself did the day before). `flowMaxWidth` capped this at whole columns so that the
          leftover — up to very nearly a whole column — was freed to sit *after* the rail, which
          brought the rail to rest one `gap-6` from the deck's last column wherever that column
          happened to end. The reader's call reverses it in both views at once: the piles played
          beside the deck are looked for **on the right**, so the leftover is dead desk inside this
          box instead, between the last column and the rail. The columns themselves are untouched —
          they are a fixed width with a fixed gutter and the pack knows nothing about either of
          these boxes.
      ...
      {rail.length > 0 && (
        <div
          {...{ [RAIL_ATTR]: "" }}
          style={{ width: COLUMN_WIDTH, flex: `0 0 ${COLUMN_WIDTH}` }}
          // **`ml-auto` is back as of 2026-08-18, and it acts only on the wrapped line.** Beside
          // the columns it resolves to zero — the flowing box is `flex-1` with no cap, so
          // `flex-grow` has already taken every free pixel and this box is at the right edge
          // because nothing is left of the line to its right. Where free space survives is the
          // line the rail wraps onto: too narrow for a column beside it, the rail goes *under* the
          // deck, and this holds it at the right edge there rather than at the left under the
          // first column, which is where 2026-08-17 left it. `StackView` carries the whole
