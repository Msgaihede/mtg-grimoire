    ...
    docs: {
      description: {
        component:
          ...
          "stale.\n\n" +
          "**The page opens on 38 cards where `Search/Page` opens on 33, and the difference is " +
          ...
          "the motif they searched for. Art results are printings. The corpus is 43 printings, " +
          ...
          "is 43 → 41 → **38**. Measured 2026-08-20 by calling " +
          "`readHandlers(seed(\"starter\")).search_cards` with this page's own request.\n\n" +
          "**The fake's motifs are the ones its 43 illustrations actually carry, and there is no " +
...
export const Default: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(await canvas.findByText("38 cards", CAPTION)).toBeInTheDocument();
    await expect(
...
export const AMotifAndItsFloor: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await canvas.findByText("38 cards", CAPTION);
...
export const ArtTagsFetchError: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.queryByText(/tags have not been downloaded yet\./)).toBeNull();
    await expect(canvas.getByText("38 cards", CAPTION)).toBeInTheDocument();
  },
};
