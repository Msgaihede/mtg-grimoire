...
export function useCardFacets(req: FacetRequest): FacetResponse | undefined {
  const query = useQuery({
    ...
    // empty corpus that a first run sits in for its whole opening sync (~20 s *inferred* on a
    // release build 2026-08-11 from when the index went ready; the ~93 s in CLAUDE.md is a
    // debug figure), and a build
