...
const meta = {
  ...
  decorators: [
    ...
    // half of it.** In table view the page is `h-full`, so it needs a parent with a height or the
    // virtualiser is handed a 0px window; in grid view the wall takes `CardGrid`'s `grow` (since
    // 2026-09-03), is as tall as its rows, and asks the nearest *scrolling* ancestor to scroll them
    // — which in the app is `main` and here is this. The class was owed from that commit and landed
    // on 2026-09-08 with the collection's and the wishlist's, which take the same change; without
    // it the walk finds nothing, falls back to the wall itself, and the story draws every row of
    // the fixture in a box that overflows its own frame. `relative` is the rule that goes with any
    // `overflow` in this app (`src/CLAUDE.md`): a scroll container has to be the containing block
...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(await canvas.findByText("37 cards")).toBeInTheDocument();
...
export const TableView: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(table).toHaveAttribute("aria-rowcount", "38");
  },
};
 ...
 * seed answers the same 37 cards `starter` does (measured 2026-08-22 over both seeds), and if a
...
/**
 * 5 246 printings of 687 cards — and the count that stops before it has walked them.
 *
 * The seed holds 5 252 rows; the two the search never counts are digital, and the four it
 ...
 *
 * All printings is pressed first, because the cap is only reachable over printings now — 687
