 ...
 * it.**
 *
 * ## Why this is a module and not five literals
 *
 * Five surfaces ask for the same card at the same moment: the card modal itself, and the four
 * overlays a reader opens from its rail — `LegalityDialog`, `CardTextDialog`, `OracleTagsDialog`
 * and `CombosDialog`. Each of the first three used to spell `["card", cardId, marketplace]` out
 * for itself, and each did so deliberately: the original lived in `CardDetailPane.tsx`, a file
 * that was deleted on 2026-09-03 once the modal's parts had moved out of it, and an import of
 * something with a demolition date is a worse dependency than a copy. That reasoning was right
 * about each file and wrong about the end state, because these are not five keys that happen to
 * look alike — they are **one cache entry that five surfaces are counting on.**
 *
 * **What the shared key buys is a round trip that never happens.** The modal has already fetched
 * the card by the time any of the four overlays can be opened, so an overlay mounting an
 ...
 *
 * **And what a drifted copy costs is exactly that, silently.** Nothing about a sixth spelling is
 ...
 * why the four spellings became one — and why the fifth surface was born reading this module
 * rather than adding a spelling of its own.
 *
 ...
 * marketplace switch would have nothing to refetch. Nothing in the four overlays draws money,
 ...
 *
 * @param cardId The printing being read. **Nullable because four of the five callers are mounted
