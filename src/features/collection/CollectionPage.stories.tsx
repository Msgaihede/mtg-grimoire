...
/**
 * The same page over a collection nobody typed in — **the sum of the reader's live deck lists**.
 *
 * `deck_driven_collection` is on and there is not one `collection_entries` row in the world
 * (`.storybook/fake/seeds.ts`'s `deckDriven` seed), so every row on screen came out of
 * `collection_source::rows`' grouped subquery. Five of them, from eight deck cards across three
 * decks: Lightning Bolt is **seven copies over all three** — four in a deck that is played, two
 * in a Maybeboard that is **switched off**, one in a deck that is **archived**, and none of the
 * three subtracted, because an inactive pile is a statement about how a *deck* is read and
 * archiving is filing rather than disassembling. Urza's Saga is two rows of one printing, the
 * foil and the regular, because `coalesce(finish, 'nonfoil')` is in the grouping key. Dismember
 * sits in a `theory` list and is **not on this page at all**: a plan is a card the reader has
 * said they do not have yet.
 *
 * Three things about the *page* change with the mode, and the play below is each of them:
 *
 * * **the note above the filter bar**, which is how somebody who forgot the switch finds it
 *   again — nothing else on screen says the list means something different now;
 * * **the Actions column**, which loses its only control (a derived row cannot be removed — the
 *   copies are in the decks) and answers the question a derived row actually raises instead:
 *   `3 decks`, with the names one hover away through `collection_row_decks`;
 * * **the stepper**, greyed with its reason in its accessible name rather than removed. It is
 *   the *second* fence: `collection::set_quantity` refuses in Rust as well, because the id on a
 *   derived row is a `deck_cards.id` and the reader's hidden hand-built rows are still on disk.
 *
 * The Condition chip group and the `· condition` half of the third column's header go with them
 * — a derived row states no grade, and a filter over a column that can only be empty matches
 * everything or nothing.
 */
export const DeckDriven: Story = {
  args: { view: "table" },
  parameters: { fake: { seed: "deckDriven" } },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    // Five derived rows plus the header. Named as well as counted, because the count alone would
    // pass over a page that summed the theory row in and dropped one elsewhere.
    await expect(await canvas.findByText("Lightning Bolt")).toBeInTheDocument();
    await expect(canvas.getByRole("table", { name: "Your collection" })).toHaveAttribute(
      "aria-rowcount",
      "6",
    );
    // The plan, absent — the one clause of `collection_source::LIVE` that no row on the page can
    // demonstrate by being on it.
    await expect(canvas.queryByText("Dismember")).toBeNull();

    await expect(
      canvas.getByText(/Your collection is the sum of the cards in your decks\./),
    ).toBeVisible();
    await expect(canvas.getByRole("button", { name: "Change this in Settings" })).toBeVisible();

    // Where the delete button was. Plural, and three because the switched-off Maybeboard and the
    // archived deck both count.
    await expect(canvas.getByText("3 decks")).toBeVisible();
    await expect(
      canvas.getByText(
    ...
    // `(Nonfoil)` and not `(Nonfoil, NM)`: `copyLabel` reads the row's own `condition`, which is
    // NULL on every derived row, so the grade drops out of the name as well as out of the cell.
    const label = "Quantity of Lightning Bolt (Nonfoil)";
    const box = canvas.getByRole("spinbutton", {
      name: `${label} — Your collection is driven by your decks`,
    });
    await expect(box).toHaveValue(7);
    await expect(box).toHaveAttribute("aria-disabled", "true");
    // `aria-disabled`, never `disabled` — the reason has to stay reachable, which means the
    // control has to stay in the tab order. Both buttons carry the same clause.
    await expect(box).not.toBeDisabled();
    await expect(
      canvas.getByRole("button", {
        name: `Decrease ${label} — Your collection is driven by your decks`,
      }),
